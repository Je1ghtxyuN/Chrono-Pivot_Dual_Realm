...
public class NextPageButtonController : MonoBehaviour
{
    [Header("VR交互设置")]
    ...
    private void Start()
    {
        ...
        if (button == null)
        {
            Debug.LogError("下一页按钮未找到！");
            return;
        }

        button.onClick.AddListener(OnNextPageClicked);
    ...
    {
        FindObjectOfType<VRBookController>()?.GoToNextPage();
    }
}