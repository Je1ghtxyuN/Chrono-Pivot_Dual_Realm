...
public class PrevPageButtonController : MonoBehaviour
{
    [Header("VR交互设置")]
    ...
    private void Start()
    {
        ...
        if (button == null)
        {
            Debug.LogError("上一页按钮未找到！");
            return;
        }

        button.onClick.AddListener(OnPrevPageClicked);
    ...
    {
        FindObjectOfType<VRBookController>()?.GoToPrevPage();
    }
}